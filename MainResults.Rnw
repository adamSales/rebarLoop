...
<<seed,echo=TRUE,cache=TRUE>>=
set.seed(613)
@

<<librarysEtc,echo=TRUE,results='hide',cache=TRUE>>=
library(scales)
#library(tidyverse)
library(dplyr)
library(ggplot2)
...
<<loadData,echo=TRUE,results='hide',cache=TRUE>>=
source('code/dataPrep.r')
@

Replicating Table \ref{tab:covariates} from the manuscript:

<<table1,echo=TRUE,cache=TRUE,results='asis'>>=
...
<<imputation,results='hide',cache=TRUE>>=
...
experiment.
<<figPrep,results='asis',cache=TRUE>>=
